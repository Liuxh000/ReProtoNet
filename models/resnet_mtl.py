...
class ResNetMtl(nn.Module):
    ...
    def __init__(self, layers=[3, 4, 6, 3], input_size=64, mtl=True):
        ...
        if mtl:
            ...
            block = BasicBlock
        cfg = [64, 128, 256, 512]
        self.inplanes = iChannels = input_size
        self.conv1 = self.Conv2d(3, iChannels, kernel_size=7, stride=2, padding=3)
        self.bn1 = nn.BatchNorm2d(iChannels)
        self.relu = nn.ReLU(inplace=True)
        self.maxpool = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)
        self.layer1 = self._make_layer(block, cfg[0], layers[0], stride=1)
